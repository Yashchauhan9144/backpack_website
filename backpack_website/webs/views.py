...
def forgetpass(request):
    ffuser=request.POST['usern']
    newp=request.POST['npass']
    conp=request.POST['nnpass']

    if newp==conp:
        obj=signupinfo.objects.get(uname=ffuser)
        obj.pswd=newp
        obj.save()

        return render(request,'myaccount.html')
    
    return HttpResponse('enter same password in input')
